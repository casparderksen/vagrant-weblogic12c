...
# create_server(name, port): create managed server
create_server() {
    wlst /vagrant/weblogic/wlst/create_server.py "$1" "$2"
}

# create_admin_user(user, password, description): create new admin user
create_admin_user() {
    wlst /vagrant/weblogic/wlst/create_admin_user.py "$1" "$2" "$3"
}
...
for server in "${servers[@]}"; do
    ...
    sed -e "s|@DOMAIN_HOME|${DOMAIN_HOME}|" \
        -e "s|@SERVER|${server}|" \
        /vagrant/weblogic/init/server.conf > /etc/init/${server}.conf
